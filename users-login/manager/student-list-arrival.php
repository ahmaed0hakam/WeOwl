...
if (!$result || mysqli_num_rows($result) == 0) {
 ...
 $stmt = $conn->prepare("SELECT * FROM student WHERE class_id = ? AND deleted = 0 ORDER BY section, first_name");
...
<html lang="en" ng-app="myApp">
  ...
  <div class="modal-content">
    ...
    <section class="section classes">
      ...
      <form id="attendanceForm" action="save-attendance-arrival.php?id=<?php echo $CLASS_ID; ?>" method="post">
      <table>
          <thead>
            <tr>
              <th>Name</th>
          ...
          if( count($attendances)>0){
            for($j = 0; $j < count($attendances);$j++){
             ...
             <?php if($students[$i]['id']==$attendances[$j]['student_id']){
                      if($attendances[$j]['arrival_status']=='Arrived'){ ?>
                        <tr>
                        ...
                        Yes <input checked class="form-check-input" type="radio" name="<?php echo $i; ?>[]" value="yes" id="flexCheckDefault" required/>
                        No <input class="form-check-input" type="radio" name="<?php echo $i; ?>[]" value="no" id="flexCheckDefault" />

              <?php }elseif($attendances[$j]['arrival_status']=='Absent'){
                ?>
                          <tr>
                          <td><?php echo $students[$i]['first_name'];?> <?php echo $students[$i]['last_name']; ?></td>
                          <td>
