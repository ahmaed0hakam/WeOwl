...
WHERE student_id=$std_id ORDER BY dates desc;
";
$r = mysqli_query($conn, $sql);
...
<html lang="en" ng-app="myApp">
  ...
  <div class="modal-content">
    ...
    <section class="section classes">
      ...
      <h2 class="title">Attendance Reports</h2>
      <p class="class">Class:  <?php echo $row['class_name']."(".$row['section'].")"; ?></p>
      <div class="buttons-scroll">
        <?php 
        for ($i = 0; $i < count($attendances); $i++) {
          ?>
          <a href="attendance-report.php?id=<?php echo $ID;?>&date=<?php echo $attendances[$i]['dates'] ;?>"><button><?php  echo $attendances[$i]['dates'];?></button></a>
     <?php } ?>
